...
f=0.005; % Grating cycles/pixel
cyclespersecond=1; % Speed of grating in cycles per second
angle=0; % Angle of grating
gridsizes = [1,2,3,4,6,8,12,16,24,32,48,64]; % Benchmark grid sizes
...
try
    ...
    for gridsize = gridsizes
        ...
        sizeX = width/gridsize;
        sizeY = height/gridsize;
...
%
    ...
    % Calculate parameters of the grating:
        ...
        while (vbl < vblendtime) && ~KbCheck
            ...
            for k = 0:(gridsize*gridsize)-1
              xoff = sizeX * mod(k,gridsize);
              yoff = sizeY * floor(k / gridsize);
              srcRect=[0 0 visiblesize visiblesize];
              dstRect = [xoff yoff sizeX+xoff sizeY+yoff];
              % Draw first grating texture, rotated by "angle":
              Screen('DrawTexture', w, gratingtex, srcRect, dstRect, angle, [], [], [], [], [], [0, yoffset, 0, 0]);
            end
